(function(owner, window, document, undefined) {
    ...
    var handleFilePath = owner.handleFilePath = function(url) {
        if (!url) return null;
    ...
    (function() {
        ...
        XMLHttpRequest.prototype.open = function() {
            ...
            if (url.indexOf('/ws/target') < 0 &&
                ...
                url.indexOf('/-dev') < 0) {
                style.href = url;
                xhrUrlMap[style.href] = xhrUrlMap[style.href] || 0;
                xhrUrlMap[style.href] += 1;
            }
    ...
    var getPageInfo = owner.getPageInfo = function() {
        var pageInfo = {};
        pageInfo.url = handleFilePath(location.href);
        pageInfo.files = [];
        var scripts = getScripts();
        pageInfo.files.push.apply(pageInfo.files, scripts.map(function(item) {
            return handleFilePath(item.src);
        }));
        ...
        pageInfo.files.push.apply(pageInfo.files, styles.map(function(item) {
            return handleFilePath(item.href);
        }));
